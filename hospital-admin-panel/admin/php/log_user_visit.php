...
$today = date('Y-m-d');
$check_sql = "SELECT id FROM user_visits WHERE user_ip = ? AND DATE(visit_time) = ? AND page_name = ? LIMIT 1";
...
if ($check_stmt->num_rows === 0) {
    ...
    $check_stmt->close();
    $insert_sql = "INSERT INTO user_visits (user_ip, page_name, visit_time) VALUES (?, ?, NOW())";
    ...
    $check_stmt->close();
    $update_sql = "UPDATE user_visits SET visit_time = NOW() WHERE user_ip = ? AND DATE(visit_time) = ? AND page_name = ?";
